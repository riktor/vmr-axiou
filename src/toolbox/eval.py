...
def axiou(
    ...
    results = []
    keyed_gt = dict(groundtruth)
    keyed_pred = dict()
    for pred_instance in prediction:
        query, pred_locs, rating = pred_instance
        keyed_pred[query] = (pred_locs, rating)

    for query, gt_loc in keyed_gt.items():
        metrics = {}
        if query not in keyed_pred:
            for k in top_k:
                metrics[f"AxIoU@{k}"] = False
            results.append((query, None, None, metrics))
            continue
        
        pred_locs, rating = keyed_pred[query]
        if pred_locs is None:
            results.append((query, None, None, {f'AxIoU@{k}': 0.0 for k in top_k}))
            continue
        ...
        for k in top_k:
            rels = np.maximum(overlap[:k] - iou_lb, 0) / (1 - iou_lb)
            metrics[f"AxIoU@{k}"] = 0
            for _k in range(1, k+1):
                metrics[f"AxIoU@{k}"] += np.max(rels[:_k]) / k
        results.append((query, pred_locs, rating, metrics))
